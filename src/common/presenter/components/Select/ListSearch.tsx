...
const ListSearch = ({value, setValue} : ListSearchProps) => {
  return (
    <div className="sticky top-0 flex flex-row h-10 bg-secondary">
      <div className="p-2 w-11/12">
        <input
            type="text"
            value={value}
            placeholder="Buscar"
            className="bg-secondary w-full text-sm text-left placeholder:text-gray-400 outline-none"
            onChange={(e) => setValue(normalizeText(e.target.value))
            }
        />
      </div>
      <div className="relative w-1/12 bg-secondary h-10">
